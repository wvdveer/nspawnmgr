 ...
 * consistently both by this check and later by {@code application.yml}'s {@code ${DB_URL:...}}
 * -style placeholders. Deliberately has zero Spring dependency — this module has no Spring
 * dependency at all. A twin copy of this exact class lives in the main nspawnmgr module too (used
 * by {@code NspawnmgrApplication#onStartup}'s own reachability pre-check, before any Spring
 * context exists there either) — this module gets its own separate classloader, so there's no way
 * to share one copy between the two WARs; small, rarely-changing, Spring-free classes like this one
 * are deliberately duplicated across this project's sibling WAR modules rather than shared (see
 * {@code SetupWizardHtml}'s own javadoc for the same convention elsewhere).
 */
public record DbConnectionSettings(String url, String username, String password, String vendor) {
     ...
     * source." Getting this wrong here would make this check reject a config Spring's real startup
     * would have happily accepted — exactly the DB_PASSWORD="" case that broke the real (non-dev)
     * container-lifecycle CI job the first time it ever ran against this code path.
     */
