...
-- script_body: VARCHAR, not LONGTEXT/TEXT - matches ContainerScript.scriptBody's plain (non-@Lob)
-- String mapping (see that field's own comment for why); 16000 chars is comfortably under MySQL's
-- ~65535-byte VARCHAR cap even at utf8mb4's worst-case 4 bytes/char.
CREATE TABLE container_scripts (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    container_id BIGINT NOT NULL,
    name VARCHAR(255) NOT NULL,
    script_body VARCHAR(16000) NOT NULL,
    created_at DATETIME NOT NULL,
    updated_at DATETIME NOT NULL,
